...
function generateExportText(summary: SessionSummary): string {
  ...
  if (cs.allergies.length) {
    lines.push(`⚠️ Allergies: ${cs.allergies.join(", ")}`);
  }
  if (cs.vitals.length) {
  ...
  if (summary.flags.length) {
    lines.push("", "─── FLAGS & WARNINGS ───", "");
    summary.flags.forEach((f) => lines.push(`⚠️ ${f}`));
  }
  ...
  lines.push(
    ...
    "",
    "Generated by MedInter on NVIDIA DGX Spark GB10",
    "═══════════════════════════════════════",
  );
...
export function ExportButton({ summary }: { summary: SessionSummary }) {
  const handleExport = () => {
    ...
    navigator.clipboard.writeText(text).then(() => {
      alert("Summary copied to clipboard — paste into EMR");
    }).catch(() => {
      ...
      a.href = url;
      a.download = `medinter-${summary.session_id}.txt`;
      a.click();
